...
class AuthController
{
    ...
    public function login()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $result = mysqli_query($conn, $querynim);
            ...
            if (mysqli_num_rows($result) == 1) {
                if (password_verify($password, $data['password'])){
                    $_SESSION['login'] = true;
                    $_SESSION['data_mahasiswa'] = $data_mahasiswa['data_mahasiswa'];
                    echo "Login Successful";
                    if (isset($input['remember'])){
                        setcookie("nim", $nim["nim"], time() + (86400 * 30), "/");
                        setcookie("password", $password["password"], time() + (86400 * 30), "/");
                        }else{
    ...
    private function generateNIM($id_pendaftaran){
        ...
        $query = mysqli_fetch_assoc($id_pendaftaran);
        ...
        $data_pendaftaran = mysqli_fetch_assoc();
